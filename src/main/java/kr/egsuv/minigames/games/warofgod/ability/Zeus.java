...
/*
    ...
    public void onHitted(EntityDamageByEntityEvent event) {
        ...
        if (event.getCause() == EntityDamageEvent.DamageCause.LIGHTNING && event.getDamager() instanceof LightningStrike) {
            if (event.getEntity() instanceof Player) {
                Player damagedPlayer = (Player) event.getEntity();

                // 제우스가 호출한 번개에 의한 것인지 확인 (여기서는 모든 번개 피해에 대해 적용)
                double damage = 12.0;  // 고정 피해
